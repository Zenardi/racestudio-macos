...
#[test]
fn test_eval_math_channel_multichannel() {
    // A two-channel expression exercises the anchor + resample-the-others path.
    let Some(path) = xrk_or_skip() else { return };
    let handle = open_session(path.to_string_lossy().into_owned()).expect("open real");
    let session = decode_session(&path).expect("decode");
    ...
    let (Some(ax), Some(ay)) = (channel("AccelerometerX"), channel("AccelerometerY")) else {
        return; // fixture without these channels — skip
    };

    let out = handle
        .eval_math_channel("AccelerometerX + AccelerometerY".into(), all())
        .expect("multi-channel eval");

    assert_eq!(out.len(), ax.len(), "one sample per anchor point");
    assert!(out.iter().all(|s| s.value.is_finite()), "no NaN/Inf leaked");
    // The two accelerometer channels share a timebase, so no interpolation is
    // needed and the result is the exact element-wise sum.
    if ax.len() == ay.len() && ax.iter().zip(&ay).all(|(a, b)| a.0 == b.0) {
        for (sample, (&(t, x), &(_, y))) in out.iter().zip(ax.iter().zip(&ay)) {
            assert!((sample.timecode - t).abs() < 1e-9);
            assert!((sample.value - (x + y)).abs() < 1e-9, "value = Ax + Ay");
        }
    }
}
...
/// Load the committed fft golden (generated by `generate_fft_golden` below).
///
/// Parsed by hand rather than with serde_json: the FFI crate is kept
/// dev-dependency-free because adding one makes cargo-llvm-cov instrument an
/// extra uncovered copy of the lib (see Cargo.toml). The golden is a flat,
/// generator-controlled shape, so a scoped scan of each key is sufficient.
fn load_fft_golden() -> Option<FftGolden> {
    let text = std::fs::read_to_string(fft_golden_path()).ok()?;
    // Scope the search for scalar keys to before the "freqs" array so a value
    // inside the arrays can never be mistaken for a top-level key.
    let head = &text[..text.find("\"freqs\"").unwrap_or(text.len())];
    let str_field = |key: &str| -> Option<String> {
        let at = head.find(&format!("\"{key}\""))?;
        let q1 = head[at + key.len() + 2..].find('"')? + at + key.len() + 3;
        let q2 = head[q1..].find('"')? + q1;
        Some(head[q1..q2].to_string())
    };
    let num_field = |key: &str| -> Option<f64> {
        let at = head.find(&format!("\"{key}\""))?;
        let rest = head[at + key.len() + 2..].trim_start_matches([':', ' ']);
        let end = rest.find([',', '}', '\n']).unwrap_or(rest.len());
