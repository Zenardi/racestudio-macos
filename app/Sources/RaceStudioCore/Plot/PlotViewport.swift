...
/// (issue 4.1). Immutable: every gesture returns a new viewport.
public struct PlotViewport: Equatable, Sendable {
    ...
    /// defaults to a millionth of the bounds span (a non-zero floor that keeps
    /// zoom-in from collapsing the window).
    public init(bounds: ClosedRange<Double>, visible: ClosedRange<Double>? = nil, minSpan: Double? = nil) {
        self.bounds = bounds
        self.visible = visible ?? bounds
        self.minSpan = minSpan ?? (bounds.upperBound - bounds.lowerBound) * 1e-6
    }
    ...
    public func zoom(factor: Double, anchor: Double) -> PlotViewport {
        ...
        let newSpan = min(max(span * factor, minSpan), maxSpan)
    ...
    public func pan(by delta: Double) -> PlotViewport {
        ...
        let lower = min(max(visible.lowerBound + delta, bounds.lowerBound), maxLower)
        return PlotViewport(bounds: bounds, visible: lower...(lower + width), minSpan: minSpan)
    }
