...
<p align="center">
  ...
  <a href="https://github.com/Zenardi/racestudio-macos/releases/latest"><img src="https://img.shields.io/github/v/release/Zenardi/racestudio-macos?label=download&color=0052cc" alt="Latest release"></a>
</p>
...
> **Status:** **M0–M5 complete**; **M6 — Device Connectivity (WiFi)** feature-complete (6.1–6.7). The
...
| **M6 — Device Connectivity (WiFi)** | Direct session import from AiM loggers over WiFi. Gated by a clean-room, interoperability-only reverse-engineering decision ([ADR 0006](docs/adr/0006-device-wifi-reverse-engineering.md)) + [legal gate](docs/device/LEGAL_GATE.md) — DMCA §1201(f) / EU 2009/24/EC Art.6, a `needs-legal-review` sign-off, and a do-not-redistribute guard enforced in CI (6.1). The macOS capture harness + clean-room **protocol dissection** ([CAPTURE.md](docs/device/CAPTURE.md), [PROTOCOL.md](docs/device/PROTOCOL.md)) — STCP framing, the additive checksum, discovery + transfer chunking — landed as `racestudio-device` with replay fixtures (6.2). **Discovery** now returns a typed `Device` (name/address/port/model) from the recorded announcement, with an mDNS/Bonjour (`NWBrowser`) adapter behind a trait seam and an AP-mode gateway fallback (6.3). **Session enumeration** builds the catalog request byte-for-byte and parses the checksum-verified response into typed `SessionInfo` (id/date/laps/size/name); the recorded store was empty at capture, so the dated-record layout is a documented hypothesis pending a session-present re-capture (6.4, [#130](https://github.com/Zenardi/racestudio-macos/issues/130)). **Session download** reassembles the checksum-verified chunk stream by offset into a decodable `.xrk` — validated end-to-end by decoding the reassembled file against the M1 golden — with per-chunk retry, whole-file verification, out-of-order/duplicate/missing-chunk handling, and progress via an injected transport; only one real chunk was captured, so the multi-chunk stream is a documented hypothesis pending a session-present capture (6.5, [#133](https://github.com/Zenardi/racestudio-macos/issues/133)). **Guarded delete** is a destructive write behind layered guards: it sends **zero bytes** unless an `armed` flag is set *and* a typed confirmation matches the target's id **and** name, then sends exactly one delete frame and never blindly retries (no double-delete); no delete traffic was ever captured (empty store), so the delete opcode + ack/reject response are synthetic hypotheses over verified STCP framing, pending a real capture (6.6, [#130](https://github.com/Zenardi/racestudio-macos/issues/130)). **The device panel** ties 6.3–6.6 together in a SwiftUI window driven by a tested `DevicePanelModel` state machine (`idle → discovering → sessions → downloading/deleting → done/error`, illegal transitions unrepresentable): it lists discovered devices, renders the session table (empty-state included), advances a 0→100% download progress bar, and gates the **destructive delete** behind a dialog that only calls the guarded 6.6 API once the user re-types the session's exact name — Cancel sends nothing. All logic lives in `RaceStudioCore` (fixture-driven, device-free, ≥95% covered) with the SwiftUI shell a thin, coverage-excluded view (6.7). | 6.1 ✅ · 6.2 ✅ · 6.3 ✅ · 6.4 ✅ · 6.5 ✅ · 6.6 ✅ · 6.7 ✅ | ✅ Complete |
| **M7 — Parity, Performance & Release** | Feature parity with RaceStudio 3, performance tuning, and a packaged release. The **parity audit** (7.1) landed [docs/PARITY_MATRIX.md](docs/PARITY_MATRIX.md): every RS3 3.83.26 analysis feature mapped to its implementing milestone issue with a prioritized gap backlog, structurally validated by `parity_matrix_test.rs` so the matrix cannot rot. **Large-session performance** (7.2) landed lazy channel loading (`Container::channel_index` decodes a channel's samples only on first access, cached once), O(n) spike-preserving min/max decimation (`min_max_decimate`, surfaced to the plot layer by `ChannelViewModel`), and a Criterion bench harness with regression ceilings gated in CI. **Accessibility & localization** (7.3) added an `.xcstrings` String Catalog (en + pt-BR) parsed at runtime by `LocalizationCatalog` (shipped verbatim into `Bundle.module`, so the completeness gate runs without Xcode's `xcstringstool`), a typed `L10n` helper with locale-aware number formatting and a flagged-sentinel for missing keys, plus `AccessibilitySummary`/`ControlLabel` for VoiceOver chart summaries and control labels — all wired through the view models and gated so every en key has a non-stale pt-BR translation ([docs/ACCESSIBILITY.md](docs/ACCESSIBILITY.md)). The **user handbook** (7.5) landed [docs/handbook/](docs/handbook/index.md) — getting started/import, the analysis views, the math-channel expression language, and device download — kept honest by a dependency-free doc-lint (`handbook_links_test.rs`: every link/image resolves, every documented function is a real `expr` built-in, every worked example still evaluates through the engine) and rendered to a static site by `make docs`. Closing parity gaps, **multi-session compare** (9.1) overlays laps from several sessions across every panel (`MultiSessionModel`), and **track detection** (9.2) landed a bundled, versioned track database plus a direction-agnostic GPS→circuit matcher (`racestudio-analysis::track`) that auto-recognizes the circuit and reads the start/finish + sector gates off the definition instead of hand-placed beacons — surfaced to the app as `DetectedTrack`/`TrackDetectionModel` with a clean fall-back to beacon segmentation when no track matches. **Video sync** (9.5) ties an external session video to the analysis cursor: `VideoSyncModel` is a pure session↔video time mapping (a single clamped sync offset, cursor↔playhead in both directions, no AVKit) that a thin coverage-excluded `AVPlayer` shell binds to — scrub the plot and the footage follows, or play the footage and the linked cursor tracks it. **Video review** ([#156](https://github.com/Zenardi/racestudio-macos/issues/156)) turns that into a review workflow: the footage is *attached* to the workspace by security-scoped bookmark and persisted in the `.rsproj` schema (v5) with its alignment, synced against the **track data** — anchor a lap or sector boundary to the frame it actually starts on (exact, not capped at the ±60 s nudge), or take an opening guess from the asset's creation date versus the log's `datetimeUtc` — and reviewed **lap by lap and sector by sector**: `LapSectorTimeline` re-cuts the 8.11 base grid into absolute session-time windows per lap and split (so the review grid and the Split Times table can never disagree), and `VideoReviewModel` owns which section is under review, where that puts the cursor and playhead, whether the footage covers it, and whether the end of a section stops or loops — all pure, with the `AVPlayer` shell only applying those decisions in a first-class **Video** rail panel. **Brand & style identity** ([#141](https://github.com/Zenardi/racestudio-macos/issues/141)) established the app's visual system: a light/dark `Theme` design-token type in `RaceStudioCore` (semantic colour palette, seven-step type scale, 4-point spacing, rounding — plus `BrandColor`'s WCAG contrast math) whose every text/surface pair is *proven* ≥ WCAG AA in both appearances by `ThemeTests`, mapped onto SwiftUI by a thin `Theme+SwiftUI.swift` environment bridge and applied to the library preview pane as the reference screen ([docs/BRAND.md](docs/BRAND.md)). The **app icon** ([#142](https://github.com/Zenardi/racestudio-macos/issues/142)) draws that identity — a brand-red squircle carrying a white racing line through a telemetry apex — reproducibly from source (`scripts/gen_app_icon.swift` renders every macOS size; `scripts/gen_app_icon.sh` packs the `AppIcon.appiconset` + `AppIcon.icns`), wired into the `make run` bundle so it shows in Dock/Finder/Launchpad, with an `AppIconTests` gate proving every 16–512 pt @1x/@2x slot is present and correctly sized. **UI styling** ([#143](https://github.com/Zenardi/racestudio-macos/issues/143)) then rolled the tokens across the whole `@main` shell — library browser, analysis window (+ every panel), device panel, measures/toolbar chrome, and session summary — replacing ad-hoc `.secondary`/system styling with `Theme` colour, type, and spacing, and adding a shared `BrandStateView`/`BrandLoadingView` so every empty/loading/error state is designed (never blank) and correct in light **and** dark (presentation only — logic unchanged). The **release pipeline** ([#50](https://github.com/Zenardi/racestudio-macos/issues/50)) makes the app downloadable: a `v*` tag runs `.github/workflows/release.yml`, which re-runs the whole `make ci` gate and only then builds a universal (arm64 + x86_64) `.app`, ad-hoc signs it with the sandbox entitlements, packages a drag-to-Applications `.dmg` plus `SHA256SUMS.txt`, and publishes the GitHub Release. Scoped deliberately to **unsigned** distribution — no paid Apple certificate, no notarization, and therefore **no secrets at all**, so anyone who can push a tag can cut a release; `tests/release_test.sh` pins that and exercises the real packaging code on every PR via `scripts/release_smoke.sh --dry-run` ([docs/RELEASE.md](docs/RELEASE.md)). | 7.1 ✅ · 7.2 ✅ · 7.3 ✅ · 7.4 ✅ · 7.5 ✅ · 9.1 ✅ · 9.2 ✅ · 9.5 ✅ · 9.6 ✅ ([#156](https://github.com/Zenardi/racestudio-macos/issues/156)) · brand identity ✅ ([#141](https://github.com/Zenardi/racestudio-macos/issues/141)) · app icon ✅ ([#142](https://github.com/Zenardi/racestudio-macos/issues/142)) · UI styling ✅ ([#143](https://github.com/Zenardi/racestudio-macos/issues/143)) | 🔄 In progress |
...
Grab **`RaceStudio-<version>.dmg`** from the
**[latest release](https://github.com/Zenardi/racestudio-macos/releases/latest)**, open it, and drag
**RaceStudio** onto **Applications**. One universal build covers Apple Silicon and Intel; macOS 13
(Ventura) or later.

> **First launch.** The build is ad-hoc signed but **not notarized** — this project has no paid
> Apple signing certificate — so macOS quarantines the download and refuses a plain double-click.
> Open it once with **right-click → Open → Open**, or clear the flag yourself:
>
> ```sh
> xattr -dr com.apple.quarantine /Applications/RaceStudio.app
> ```

Verify what you downloaded against the release's `SHA256SUMS.txt`
(`shasum -a 256 -c SHA256SUMS.txt`). Releases are cut entirely by
[`.github/workflows/release.yml`](.github/workflows/release.yml) on a `v*` tag — it re-runs the full
`make ci` gate before it packages anything, and consumes no secrets. Full detail, including how to
build the `.dmg` locally with `make dmg`, is in [`docs/RELEASE.md`](docs/RELEASE.md).
...
core/
  ...
  racestudio-device/           # MyChron WiFi protocol (6.2) — STCP framing + additive checksum, discovery/transfer notes; typed Device discovery + AP-mode fallback (6.3); session enumeration → typed SessionInfo (6.4); chunked session download → decodable .xrk with checksum verify + progress (6.5); guarded session delete → arm + typed confirmation, zero-bytes-on-refusal, no blind retry (6.6)
app/
...
scripts/
  ...
  bench.sh                     # 7.2 Criterion benches + CI regression-ceiling gate (--ci)
  bench_thresholds.json        # 7.2 per-benchmark regression margins + wall-clock ceilings
  gen_synthetic.py             # 7.2 synthetic 5M .xrk perf fixture + decimation golden (--synthetic)
...
docs/PARITY_MATRIX.md          # RS3 parity audit (7.1): feature matrix + prioritized gap backlog
docs/BRAND.md                  # brand & style identity (7.3/#141): Theme design tokens (palette/type/spacing), WCAG AA contrast, where tokens live + how views consume them
app/AppIcon/                   # app icon (7.4/#142): AppIcon.icns (bundle) + Assets.xcassets/AppIcon.appiconset (all macOS sizes); regenerate with `bash scripts/gen_app_icon.sh`
scripts/gen_app_icon.swift     # app-icon source: draws the icon (AppKit/CoreGraphics) at any size — the reproducible source of truth for app/AppIcon/
...
| `make dmg` | package the downloadable `dist/RaceStudio-<version>.dmg` (`VERSION=1.2.0` to stamp one) |
| `make release-smoke` | verify the release packaging in seconds, without a Swift build |
