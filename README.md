...
core/
  ...
  racestudio-device/           # MyChron WiFi protocol (6.2) — STCP framing + additive checksum, discovery/transfer notes; typed Device discovery + AP-mode fallback (6.3); session enumeration → typed SessionInfo (6.4); chunked session download → decodable .xrk with checksum verify + progress (6.5)
app/
