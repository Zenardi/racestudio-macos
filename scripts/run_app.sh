...
# an un-bundled executable non-GUI (accessory) activation, so the app's
# `DocumentGroup` scene never registers a Dock icon or shows a window — the
# process just exits back to the shell. Wrapping the built binary in a minimal
# `.app` bundle (an `Info.plist` with `NSPrincipalClass = NSApplication`) and
# `open`-ing it gives regular foreground activation: the app appears in the Dock
# and shows its document window.
#
